    ...
    return (
        <AnimatePresence>
            {isOpen && profile?.avatar_url && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 cursor-pointer" onClick={onClose}>
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    <motion.div
                        ...
                        className="absolute right-8 top-1/2 -translate-y-1/2 hidden md:flex items-center gap-3 text-zinc-400 pointer-events-none z-50 mix-blend-difference"
                    >
                    ...
                    <motion.div
                        ...
                        exit={{ opacity: 0, transition: { duration: 0 } }}
                        transition={{ duration: 4.5, times: [0, 0.1, 0.8, 1], ease: "easeInOut" }}
                        className="absolute left-8 top-1/2 -translate-y-1/2 hidden md:flex flex-row-reverse items-center gap-3 text-zinc-400 pointer-events-none z-50 mix-blend-difference"
                    >
                        <span className="text-sm font-medium tracking-widest uppercase">Click outside to close</span>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5 animate-pulse"><path strokeLinecap="round" strokeLinejoin="round" d="M15.042 21.672 13.684 16.6m0 0-2.51 2.225.569-9.47 5.227 7.917-3.286-.672ZM12 2.25V4.5m5.834.166-1.591 1.591M20.25 10.5H18M7.757 14.743l-1.59 1.59M6 10.5H3.75m4.007-4.243-1.59-1.59" /></svg>
                    </motion.div>

                    {/* Independent Close Button (Not clipped by circle) */}
                    <motion.button
                        exit={{ opacity: 0, transition: { duration: 0 } }}
                        onClick={onClose}
                        className="absolute top-4 right-4 md:top-8 md:right-8 w-12 h-12 rounded-full bg-black/50 backdrop-blur-md text-white border border-white/10 flex items-center justify-center hover:bg-black/70 transition-colors z-[130] cursor-pointer"
                    >
                    ...
                    <motion.div
                        ...
                        className="relative aspect-square w-full max-w-[600px] overflow-hidden rounded-full border border-zinc-800 shadow-2xl bg-zinc-950 cursor-default"
                    >
